...
public class HometBoardController {
   ...
   public String board_flist(String page,Model model)
   {
	   ...
	   map.put("start", start);
	   map.put("end", end);
	   List<HometBoardVO> list=hDao.freeboardListData(map);
	   int totalpage=hDao.freeboardTotalPage();
   ...
   {
	   // 데이터 읽기 => DAO연결 
	   HometBoardVO vo=hDao.freeboardDetailData(no);
   ...
   {
	   // 결과값 
	   HometBoardVO vo=hDao.freeboardUpdateData(no);
	   // 데이터연동 (DAO)
	   // 데이터를 JSP로 전송
	   model.addAttribute("vo", vo);
   ...
   {
	   //데이터베이스 연동 
	   boolean bCheck=hDao.freeboardUpdate(vo);
   ...
   public String board_fdelete_ok(int no,String pwd,Model model)
   {
	   ...
	   boolean bCheck=hDao.freeboardDelete(no, pwd);
   ...
   public String board_reply_to_reply(int pno,int bno, String msg,RedirectAttributes ra,HttpSession session)
   {
	   ...
	   vo.setBno(bno);
	   vo.setMsg(msg);
	   hrDao.replyToReplyInsert(pno, vo);
   ...
   {
	   List<HometBoardReplyVO> rList=hrDao.replyListData(bno);
	   model.addAttribute("rList", rList);
